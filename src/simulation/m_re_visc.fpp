...
!> @brief The module contains routines that compute viscosity-related quantities:
!!
!!  1. s_compute_re_visc: returns Re_visc_per_phase(fluid, dir) = 1/mu.
!!     USE FOR: mixture Re (CFL / viscous dt), stress terms, and Riemann solvers.
!!     Callers form mixture Re as 1/sum(alpha/Re_visc_per_phase).
!!
!!  2. s_compute_phase_viscosity_at_cell: returns mu (viscosity) per phase.
!!     USE FOR: gradients of viscosity (dmu/dx, dmu/dy, dmu/dz).
module m_re_visc
...
contains
  ...
  !! Uses 2nd order central differences in interior, 1st order one-sided at boundaries
  ...
  !> Main interface: Computes Re_visc per-phase for both Newtonian and non-Newtonian fluids
  !! @param q_prim_vf Primitive variables
  ...
  !> Computes per-phase viscosity (mu) at a single cell
  !! Returns mu (viscosity), not 1/mu. Used for grad(mu) computation.
  !! @param q_prim_vf Primitive variables
  !! @param j x index
  !! @param k y index
  !! @param l z index
  ...
  !! @param grad_y_vf Optional pre-computed y-direction gradients
  !! @param grad_z_vf Optional pre-computed z-direction gradients
  pure subroutine s_compute_phase_viscosity_at_cell( &
      q_prim_vf, j, k, l, mu_shear_phases, mu_bulk_phases, &
      grad_x_vf, grad_y_vf, grad_z_vf)
    $:GPU_ROUTINE(parallelism='[seq]')
    ...
    integer, intent(in) :: j, k, l
    #:if not MFC_CASE_OPTIMIZATION and USING_AMD
        real(wp), dimension(3), intent(out) :: mu_shear_phases, mu_bulk_phases
    #:else
